...
static void help_exit( const char *name,  char *err )
{
    ...
    printf("\nBy default runs all dictionary tests\n");
    exit( (err != NULL) );
}
...
int main( int argc, const char **argv )
{
    ...
    if ( 0 != chdir( "test/dictionaries" ) ) {
        printf("%s: Unable to find dictionary files - exiting\n",
                args.xname );
        return 1;
    }

    test_t tests[] = { { "small-dict.json", true }, {"medium-dict.json", true }, { "large-dict.json", true } };
